...
pdf("Outputs/Decay_Scatter_Hists.pdf")
  scatterhist(x = data$degree_decay_rate,y = data$Decay_Rate_per_day, xlab.use = expression(K[dd]), ylab.use = expression(K[cd]))
dev.off()
...
pdf("Outputs/Decay_v_Temp.pdf")
  scatter.no.hist(x = data$sum_mean_daily_temp,y = data$Decay_Rate_per_day,xlab.use = expression(Summed~Temperature~(degree*C)),ylab.use = expression(K[cd]))
dev.off()
